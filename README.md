...
기능적 요구사항
1. 고객이 동물병원에 예약한다. (또는 예약 후 진료를 받는다)
1. 수의사는 동물을 진료/처방한다. 
1. 수의사는 수납에게 진료비 산정을 요청한다.
1. 수납은 진료비 산정 후 고객에게 진료비를 청구한다.
1. 고객이 치료비를 지불한다.
1. 고객이 예약을 변경/취소할 수 있다
1. 예약이 변경/취소되면 수의사 배당이 변경/취소된다. (삭제 가능)
...
1. 트랜잭션
    1. (없어 보이나 있으면 추가 바람) 
1. 장애격리
    1. 수의사배정 기능이 수행되지 않더라도 예약은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
    1. 예약시스템이 과중되면 사용자를 잠시동안 받지 않고 예약을 잠시후에 하도록 유도한다  Circuit breaker, fallback
1. 성능
    1. 고객이 진료/치료 결과를 수납시스템에서 확인할 수 있어야 한다.  CQRS
