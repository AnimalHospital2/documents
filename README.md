...
분석단계에서의 조건 중 하나로 예약(reservation)-> 진단(diagnosis) 간의 호출은 동기식 일관성을 유지하는 트랜잭션으로 처리하기로 함. 호출 프로토콜은 이미 앞서 Rest Repository 에 의해 노출되어있는 REST 서비스를 FeignClient 를 이용하여 호출하도록 함.

- 진단서비스를 호출하기 위하여 Stub과 (FeignClient) 를 이용하여 Service 대행 인터페이스 (Proxy) 를 구현 
...
진료 후에 결제(Acceptance) 시스템, CQRS(Lookup)으로 이를 알려주는 행위는 동기식이 아니라 비 동기식으로 처리하여 수납처리를 위한 결제주문이 블로킹 되지 않아도록 처리한다.
...
- 수납 서비스에서는 진단완료 이벤트에 대해서 이를 수신하여 자신의 수납 업무를 구현한다:
...
시나리오는 예약(reservation)-> 진단(diagnosis) 간의 호출은 RESTful Request/Response 로 연동하여 구현이 되어있고, 결제 요청이 과도할 경우 CB 를 통하여 장애격리.
